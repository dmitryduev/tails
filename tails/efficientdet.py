...
WEIGHTS_HASHES = {
    "efficientnet-b0": (
        "163292582f1c6eaca8e7dc7b51b01c61" "5b0dbc0039699b4dcd0b975cc21533dc",
        "c1421ad80a9fc67c2cc4000f666aa507" "89ce39eedb4e06d531b0c593890ccff3",
    ),
    "efficientnet-b1": (
        "d0a71ddf51ef7a0ca425bab32b7fa7f1" "6043ee598ecee73fc674d9560c8f09b0",
        "75de265d03ac52fa74f2f510455ba64f" "9c7c5fd96dc923cd4bfefa3d680c4b68",
    ),
    "efficientnet-b2": (
        "bb5451507a6418a574534aa76a91b106" "f6b605f3b5dde0b21055694319853086",
        "433b60584fafba1ea3de07443b74cfd3" "2ce004a012020b07ef69e22ba8669333",
    ),
    "efficientnet-b3": (
        "03f1fba367f070bd2545f081cfa7f3e7" "6f5e1aa3b6f4db700f00552901e75ab9",
        "c5d42eb6cfae8567b418ad3845cfd63a" "a48b87f1bd5df8658a49375a9f3135c7",
    ),
    "efficientnet-b4": (
        "98852de93f74d9833c8640474b2c698d" "b45ec60690c75b3bacb1845e907bf94f",
        "7942c1407ff1feb34113995864970cd4" "d9d91ea64877e8d9c38b6c1e0767c411",
    ),
    "efficientnet-b5": (
        "30172f1d45f9b8a41352d4219bf930ee" "3339025fd26ab314a817ba8918fefc7d",
        "9d197bc2bfe29165c10a2af8c2ebc675" "07f5d70456f09e584c71b822941b1952",
    ),
    "efficientnet-b6": (
        "f5270466747753485a082092ac9939ca" "a546eb3f09edca6d6fff842cad938720",
        "1d0923bb038f2f8060faaf0a0449db4b" "96549a881747b7c7678724ac79f427ed",
    ),
    "efficientnet-b7": (
        "876a41319980638fa597acbbf956a82d" "10819531ff2dcb1a52277f10c7aefa1a",
        "60b56ff3a8daccc8d96edfd40b204c11" "3e51748da657afd58034d54d3cec2bac",
    ),
}
...
def mb_conv_block(
    inputs, block_args, activation, drop_rate=None, prefix="", freeze_bn=False
):
...
def SeparableConvBlock(num_channels, kernel_size, strides, name, freeze_bn=False):
    f1 = tf.keras.layers.SeparableConv2D(
        num_channels,
    ...
    f2 = tf.keras.layers.BatchNormalization(
...
def ConvBlock(num_channels, kernel_size, strides, name, freeze_bn=False):
    ...
    f3 = tf.keras.layers.ReLU(name="{}_relu".format(name))
    return reduce(
...
def build_wBiFPN(features, num_channels, id, freeze_bn=False):
    if id == 0:
        ...
        P5_in_1 = tf.keras.layers.Conv2D(
            num_channels,
            kernel_size=1,
            padding="same",
            name=f"fpn_cells/cell_{id}/fnode1/resample_0_2_6/conv2d",
        )(P5_in)
        P5_in_1 = tf.keras.layers.BatchNormalization(
            momentum=MOMENTUM,
            epsilon=EPSILON,
        ...
        P4_in_1 = tf.keras.layers.Conv2D(
            num_channels,
            kernel_size=1,
            padding="same",
            name=f"fpn_cells/cell_{id}/fnode2/resample_0_1_7/conv2d",
        )(P4_in)
        P4_in_1 = tf.keras.layers.BatchNormalization(
            momentum=MOMENTUM,
            epsilon=EPSILON,
        ...
        P3_in = tf.keras.layers.Conv2D(
            num_channels,
            kernel_size=1,
            padding="same",
            name=f"fpn_cells/cell_{id}/fnode3/resample_0_0_8/conv2d",
        )(P3_in)
        P3_in = tf.keras.layers.BatchNormalization(
            momentum=MOMENTUM,
            epsilon=EPSILON,
        ...
        P4_in_2 = tf.keras.layers.Conv2D(
            num_channels,
            kernel_size=1,
            padding="same",
            name=f"fpn_cells/cell_{id}/fnode4/resample_0_1_9/conv2d",
        )(P4_in)
        P4_in_2 = tf.keras.layers.BatchNormalization(
            momentum=MOMENTUM,
            epsilon=EPSILON,
        ...
        P3_D = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="same")(
            P3_out
        )
        ...
        P5_in_2 = tf.keras.layers.Conv2D(
            num_channels,
            kernel_size=1,
            padding="same",
            name=f"fpn_cells/cell_{id}/fnode5/resample_0_2_10/conv2d",
        )(P5_in)
        P5_in_2 = tf.keras.layers.BatchNormalization(
            momentum=MOMENTUM,
            epsilon=EPSILON,
        ...
        P4_D = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="same")(
            P4_out
        )
        ...
        P5_D = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="same")(
            P5_out
        )
        ...
        P6_D = tf.keras.layers.MaxPooling2D(pool_size=3, strides=2, padding="same")(
            P6_out
        )
...
def build_BiFPN(features, num_channels, id, freeze_bn=False):
    if id == 0:
        ...
        P7_U = tf.keras.layers.UpSampling2D()(P7_in)
        P6_td = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode0/add")(
            [P6_in, P7_U]
        )
        P6_td = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P6_td)
        P6_td = SeparableConvBlock(
        ...
        # P5_in_1 = BatchNormalization(freeze=freeze_bn, name=f'fpn_cells/cell_{id}/fnode1/resample_0_2_6/bn')(P5_in_1)
        P6_U = tf.keras.layers.UpSampling2D()(P6_td)
        P5_td = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode1/add")(
            [P5_in_1, P6_U]
        )
        P5_td = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P5_td)
        P5_td = SeparableConvBlock(
        ...
        # P4_in_1 = BatchNormalization(freeze=freeze_bn, name=f'fpn_cells/cell_{id}/fnode2/resample_0_1_7/bn')(P4_in_1)
        P5_U = tf.keras.layers.UpSampling2D()(P5_td)
        P4_td = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode2/add")(
            [P4_in_1, P5_U]
        )
        P4_td = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P4_td)
        P4_td = SeparableConvBlock(
        ...
        # P3_in = BatchNormalization(freeze=freeze_bn, name=f'fpn_cells/cell_{id}/fnode3/resample_0_0_8/bn')(P3_in)
        P4_U = tf.keras.layers.UpSampling2D()(P4_td)
        P3_out = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode3/add")(
            [P3_in, P4_U]
        )
        P3_out = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P3_out)
        P3_out = SeparableConvBlock(
        ...
        P4_out = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode4/add")(
            [P4_in_2, P4_td, P3_D]
        )
        P4_out = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P4_out)
        P4_out = SeparableConvBlock(
        ...
        P5_out = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode5/add")(
            [P5_in_2, P5_td, P4_D]
        )
        P5_out = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P5_out)
        P5_out = SeparableConvBlock(
        ...
        P6_out = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode6/add")(
            [P6_in, P6_td, P5_D]
        )
        P6_out = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P6_out)
        P6_out = SeparableConvBlock(
        ...
        P7_out = tf.keras.layers.Add(name=f"fpn_cells/cell_{id}/fnode7/add")(
            [P7_in, P6_D]
        )
        P7_out = tf.keras.layers.Activation(lambda x: tf.nn.swish(x))(P7_out)
        P7_out = SeparableConvBlock(
...
class LocNet(models.Model):
    ...
    def call(self, inputs, **kwargs):
        ...
        # this will figure out the size of the second dimension at exec time
        # outputs = self.reshape(outputs)
        # make it explicit:
        n_neu = outputs.shape[1] * outputs.shape[2]
        outputs = layers.Reshape((n_neu, self.num_classes))(outputs)
...
def Tails(
    ...
    weighted_bifpn: bool = False,
    name: str = "tails",
):

    # input triplet [science, reference, difference]:
    x_input = tf.keras.Input(shape=input_shape, name="triplet")

    # EfficientNet backbone with intermediate outputs:
    # (to help accurately capture not only the classification, but also the location)
    features = EfficientNetB0(input_tensor=x_input, weights=None)

    # construct bi-directional FPN
    if weighted_bifpn:
        fpn_features = features
        for i in range(d_bifpns[0]):
            fpn_features = build_wBiFPN(fpn_features, w_bifpns[0], i)
    else:
        fpn_features = features
        for i in range(d_bifpns[0]):
            fpn_features = build_BiFPN(fpn_features, w_bifpns[0], i)

    # construct classification/localization network
    width = w_bifpns[0]
    depth = d_heads[0]
    ...
    convs = [
        tf.keras.layers.Conv2D(
            filters=width, bias_initializer="zeros", name=f"{name}/class-{i}", **options
        )
        for i in range(depth)
    ]
    # print(convs)
    ...
    bns = [
        [
            tf.keras.layers.BatchNormalization(
                momentum=MOMENTUM, epsilon=EPSILON, name=f"{name}/class-{i}-bn-{j}"
            )
            for j in range(3, 8)
        ]
        for i in range(depth)
    ...
    relu = tf.keras.layers.Lambda(lambda x: tf.nn.swish(x))
    # print(relu)

    # process levels of FPN features:
    outputs = []

    for level, feature in enumerate(fpn_features):
        # print(level, feature)
        for i in range(depth):
            feature = convs[i](feature)
            feature = bns[i][level](feature)
            feature = relu(feature)
        output = head(feature)
        n_neu = output.shape[1] * output.shape[2]
        output = tf.keras.layers.Reshape((n_neu, num_classes))(output)
        output = tf.keras.layers.Activation("sigmoid")(output)
        outputs.append(output)

    # print(outputs)

    # concatenate outputs for different levels:
    x = tf.keras.layers.Concatenate(axis=1, name="loc_features")(outputs)
    # print(x)

    # remove the last dimention
    x = tf.squeeze(x, axis=-1)

    # attach the localizing output dense layer [p_got_comet, x_relative, y_relative]
    n_output_neurons = 3
    activation = "sigmoid"
